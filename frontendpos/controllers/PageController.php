...
namespace frontendpos\controllers;
...
class PageController extends BaseController {
                        ...
                        if ($flag) {
                            if (!empty($post['payment'])) {
                                foreach ($post['payment'] as $payment) {
                                    ...
                                    } elseif ($payment['paymentMethod'] == 'XVCHR') {
            ...
            if (!$modelMtableSession->bill_printed) {
                ...
                if ($flag) {
                    ...
                    if (!empty($post['inputPayment'])) {
                        ...
                        if ($menuQueueList != '')
                            $menuQueueList .= '<br><br>Hanya bisa update discount.';
    ...
    public function actionGetMenuCategory() {
        ...
        $modelMenuCategory = $modelMenuCategory->asArray()->all();
        ...
        if (!empty(($post = Yii::$app->request->post()))) {
            ...
            if (!empty($post['menu'])) {                  
                ...
                foreach ($modelMenuQueue as $key => $menuOrder) {
                    ...
                    $orderQueue[$key]['menu'] = $menuOrder->menu->nama_menu;
    ...
    public function actionAuthorize() {
        if (!empty($post = Yii::$app->request->post())) {
            if (($modelUser = User::findOne($post['userId'])) !== null) {
                ...
                if ($modelUser->validatePassword($post['password'])) {
                    if ($modelUser->userLevel->is_super_admin) {
                        ...
                        foreach ($userAkses as $value) {
                            if (
                                    ...
                                    && $value['userAppModule']['sub_program'] === 'frontendpos') 
                                ) {
