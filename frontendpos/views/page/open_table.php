...
if (!empty($orderQueue)) {
    $jscriptOrderQueue = '
        var printOrder = function() {
            ...
            header += "Tanggal/Jam Print" + separatorPrint(14 - "Tanggal/Jam Print".length) + ": " + datetime + "\n";
            header += separatorPrint(40, "-") + "\n";
            header += "Meja" + separatorPrint(14 - "Meja".length) + ": " + $("input#tableId").val() + "\n";
            header += "Tanggal/Jam Open" + separatorPrint(14 - "Tanggal/Jam Open".length) + ": " + $("input#tglJam").val() + "\n";
            header += "Kasir" + separatorPrint(14 - "Kasir".length) + ": " + $("input#userActive").val() + "\n";                                
...
$jscript = '       
    ...
    var catatanMenuModal = function(thisObj, oldValue, title, theFunction) {
        ...
        }).attr("class", "form-control keyboard").focusin(function(event){
            openKeyboard();
        }).val(oldValue);                
    ...
    var openTable = function() {
        if ($("input#sessionMtable").length == 0) {
            ...
            }).attr("class", "form-control keyboard jmlTamu").focusin(function(event){
                openKeyboard();
            }).val($("input#inputJumlahTamu").val());
            
            var openKeyboard2 = function() {
                var kb = $(".namaTamu").getkeyboard();
                if ( kb.isOpen ) {
                    kb.close();
                } else {
                    kb.reveal();
            ...
            }).attr("class", "form-control keyboard namaTamu").focusin(function(event){
                openKeyboard2();
            }).val($("input#inputNamaTamu").val());
            ...
            $("#modalCustomNoClose #modelCustomBody #content").html("").append(label).append(jmlTamu).append("<br>").append(label2).append(namaTamu).append("<br>").append(submit);
    ...
    $("a#btnVoid").on("click", function(event) {
        ...
        var voidMenuFunction = function() {
            ...
            if ($("tbody#tbodyOrderMenu tr#menuRow").hasClass("highlight")) {            
                ...
                }).attr("class", "form-control keyboard jmlVoid").focusin(function(event){
                    openKeyboard();
                });
    ...
    $("a#btnPrintInvoice").on("click", function(event) {
        ...
        var functionPrintInvoice = function() {
            if ($(document).find("input#sessionMtable").hasClass("sessionMtable")) {
                ...
                printContent("", "", content, false, tagihanPrinted);
            } else {
    ...
    $("button#btnJumlahTamu").on("click", function(event) {
        ...
        }).attr("class", "form-control keyboard jmlTamu2").focusin(function(event){
            openKeyboard();
        }).val($("input#inputJumlahTamu").val());
        
        var openKeyboard2 = function() {
            var kb = $(".namaTamu2").getkeyboard();
            if ( kb.isOpen ) {
                kb.close();
            } else {
                kb.reveal();
        ...
        }).attr("class", "form-control keyboard namaTamu2").focusin(function(event){
            openKeyboard2();
        }).val($("input#inputNamaTamu").val());
    ...
    $("input.keyboardUserPass").keyboard({
            openOn   : null,                        
            layout: "qwerty",
            css: {
                input: "form-control input-sm",
    ...
    $("input.showKeyboard").focusin(function(event){            
